public class DList<T> {
	...
	DList()
	{
		//init vars
		size = 0;
		sentinel = new DNode<>(null, sentinel, sentinel);
	}
	...
	T value_at(int index)
	{
		...
		int counter = 1;
		DNode<T> temp = sentinel.next.next;
		//traverse list
		while(counter != index)
		{
			temp = temp.next;
	...
	void push_front(T value)
	{
		...
		if(size == 0)
		{
			sentinel.next.value = value;
			size++;
			return;
		}
		//change head with new node
		DNode<T> temp = new DNode<>(value, sentinel.next, null);
		sentinel.next = temp;
		size++;
	}
	...
	void push_back(T value)
	{
		...
		DNode<T> temp = new DNode<T>(value, null, sentinel.prev);
	...
	void insert(int index, T value)
	{
		...
		DNode<T> traverseCurrent = sentinel.next;
		DNode<T> traversePast = sentinel.next;
		int counter = 0;
	...
	T value_n_from_end(int n)
	{
		...
		DNode<T> traverse = sentinel.prev;
		int counter = 0;
		//traverse list
		while(counter < n)
		{
...
//		head = reverseHead;
	...
	void remove_value(T value)
	{
		...
		DNode<T> traverse = sentinel.next;
